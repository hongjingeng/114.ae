        ...
        <div class="music-container" id="music-app">
            ...
            <div class="music-player">
                <div style="display:flex;gap:12px;flex-wrap:wrap;">
                    <input id="music-keyword" v-model="keyword" type="text" 
                    ...
                    <button onclick="searchMusicHandler()" 
...
window.searchMusicHandler = async function() {
    ...
    try {
        ...
        const response = await fetch(proxy + `https://u.y.qq.com/cgi-bin/musicu.fcg?data={"req_1":{"method":"DoSearchForQQMusicDesktop","module":"music.search.SearchCgiService","param":{"query":"${encodeURIComponent(keyword)}","num_per_page":30}}}`);
        ...
        songs.forEach(song => {
            html += `
            <div class="song-item" onclick="playSong('${song.mid}', '${song.title}', '${song.singer ? song.singer.map(s=>s.name).join(',') : ''}')">
                <img src="https://y.gtimg.cn/music/photo_new/T002R300x300M000${song.album?.mid || '003'}.jpg" style="width:60px;height:60px;border-radius:8px;" onerror="this.src='https://picsum.photos/id/1015/60/60'">
                <div style="flex:1;">
        ...
        listEl.innerHTML = html || '<div style="padding:60px;text-align:center;color:var(--text-secondary);">未找到歌曲</div>';
    } catch(e) {
